...
Material *SceneParser::parseMaterial() {
    ...
    while (true) {
        ...
        {
            getToken(token);
            if (strcmp(token, "DIFF") == 0) type = DIFF;
            else if (strcmp(token, "SPEC") == 0) type = SPEC;
            else if (strcmp(token, "REFR") == 0) type = REFR;
            else assert (false);
        } 
        ...
        {
            reflection = readVector3f();
        } 
        else if (strcmp(token, "refRate") == 0) 
        {
            refRate = readDouble();
        } 
    ...
    auto *answer = new Material(type, color, reflection, refRate, texture);
    return answer;
}
...
Rectangle *SceneParser::parseRectangle() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Rectangle(pnt, dirx, diry, current_material);
}
