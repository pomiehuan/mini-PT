...
double UnClampColor (unsigned char color) {
...
Image* Image::LoadBMP(const char *filename)
{
    ...
    auto res = fread(&bmph.bfType, 2, 1, file);
    res = fread(&bmph.bfSize, 4, 1, file);
    res = fread(&bmph.bfReserved, 4, 1, file);
    res = fread(&bmph.bfOffBits, 4, 1, file);
    res = fread(&bmph.biSize, 4, 1, file);
    res = fread(&bmph.biWidth, 4, 1, file);
    res = fread(&bmph.biHeight, 4, 1, file);
    res = fread(&bmph.biPlanes, 2, 1, file);
    res = fread(&bmph.biBitCount, 2, 1, file);
    res = fread(&bmph.biCompression, 4, 1, file);
    res = fread(&bmph.biSizeImage, 4, 1, file);
    res = fread(&bmph.biXPelsPerMeter, 4, 1, file);
    res = fread(&bmph.biYPelsPerMeter, 4, 1, file);
    res = fread(&bmph.biClrUsed, 4, 1, file);
    res = fread(&bmph.biClrImportant, 4, 1, file);
    ...
    {
        res = fread(line, bytesPerLine, 1, file);
        for (int j = 0; j < width; j++)
        {
            int c_map_index = (width * i + j);
            double r = UnClampColor(line[3*j+2]);
            double g = UnClampColor(line[3*j+1]);
            double b = UnClampColor(line[3*j]);
